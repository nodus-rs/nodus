...
1. Decide whether this should be a one-stop global install or a project-scoped install.
   - If the user is a beginner, just wants to get started, and does not ask for a different package, prefer `WendellXY/nodus --global`.
...
- For beginner users, prefer one clear recommendation. Default to `nodus add WendellXY/nodus --global --adapter <adapter>` unless they clearly want project-scoped setup.
